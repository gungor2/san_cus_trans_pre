                ...
                for bin_i in bins:
                    ...
                    for fold_, (trn_idx, val_idx) in enumerate(folds.split(feature_tr.values, target_tr.values)):
                            ...
                            for i in range(len(cols_)):
                                var = cols_[i]
                                print(var)
                                flag_temp = target_tr.iloc[trn_idx] ==1
                                data_t = feature_tr_tr[var].loc[flag_temp]
                                hist, bin_edges = np.histogram(data_t, bins=bin_i, density=True)
                    ...
                    if temp>auc_max_f:
                            auc_max_f = temp
                            clf_f_best.save_model('best_grid_fe.txt')
                            best_LR = LR
                            best_NL = NL
                            best_SS = SS
                            best_CS = CS
                    ...
                    auc_all.append(temp)
                    print( 'LR is ' + str(LR) + ' NL is ' + str(NL) + ' SS is ' + str(SS) + ' CS is ' + str(CS) + ' Bin is ' + str(bin_i))
                    
                    print( 'Best LR is ' + str(best_LR) + ' Best NL is ' + str(best_NL) + ' Best SS is ' + str(best_SS) + ' Best CS is ' + str(best_CS) 
                    + ' Best Bin is ' + str(best_bin) )
                    print('Best AUC is ', auc_max_f)
                    print(auc_all)
